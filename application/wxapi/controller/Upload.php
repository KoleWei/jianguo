...
class Upload extends Api
{
    ...
    public function thumbnail()
    {
        ...
        $savekey = $upload['savekey'];
        $savekey = str_replace(array_keys($replaceArr), array_values($replaceArr), $savekey);
        $thumbnailsavekey = str_replace(array_keys($replaceArr), array_values($replaceArr), '/uploads/{year}{mon}{day}/{filemd5}_sm{.suffix}');

        $uploadDir = substr($savekey, 0, strripos($savekey, '/') + 1);
        $fileName = substr($savekey, strripos($savekey, '/') + 1);
        $thumbnailsuploadDir = substr($thumbnailsavekey, 0, strripos($thumbnailsavekey, '/') + 1);
        $thumbnailsfileName = substr($thumbnailsavekey, strripos($thumbnailsavekey, '/') + 1);
        //
        $splInfo = $file->validate(['size' => $size])->move(ROOT_PATH . '/public' . $uploadDir, $fileName);
        if ($splInfo) {
            $params = array(
            ...
            $thumbnailsSavePate = $uploadDir . $splInfo->getSaveName();
